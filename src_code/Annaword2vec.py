...
try:
    #make sure edited version of word2vec.inner is in place
    from gensim.models.word2vec_inner import train_batch_sg, train_batch_cbow
    from gensim.models.word2vec_inner import score_sentence_sg, score_sentence_cbow
    from gensim.models.word2vec_inner import FAST_VERSION, MAX_WORDS_IN_BATCH
except ImportError:
    # failed... fall back to plain numpy (20-80x slower training than the above)
    print 'import error in word2vec inner that you have modified'
    raw_input()
    FAST_VERSION = -1
    ...
    def train_batch_sg(model, sentences, alpha, work=None):
        ...
        #             print 'pos2 and word2', pos2, word2
        #             if pos2 != pos:
        #                 print 'calling train_sg_pair'
        #                 print model
        #                 print model.index2word[word.index]
        #                 print word2.index
        #                 print alpha
        #                 raw_input()
        ...
        for sentence in sentences:
            ...
            if len (word_vocabs) > 1:
                for word in word_vocabs[:1]: #first word in the sentence is the root of neihood
                    for word2 in word_vocabs[1:]:#window size is dynamic
                        train_sg_pair(model, model.index2word[word.index], word2.index, alpha)
            result += len(word_vocabs)
